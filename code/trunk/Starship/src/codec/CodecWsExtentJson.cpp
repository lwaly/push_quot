...
//http parse
int CodecWsExtentJson::OnMessageBegin(http_parser *parser)
{
...
int CodecWsExtentJson::OnUrl(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnStatus(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnHeaderField(http_parser *parser, const char *at,
                size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnHeaderValue(http_parser *parser, const char *at,
                size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnHeadersComplete(http_parser *parser)
{
...
int CodecWsExtentJson::OnBody(http_parser *parser, const char *at, size_t len)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnMessageComplete(http_parser *parser)
{
    HttpMsg* pHttpMsg = (HttpMsg*) parser->data;
...
int CodecWsExtentJson::OnChunkHeader(http_parser *parser)
{
    return (0);
}

int CodecWsExtentJson::OnChunkComplete(http_parser *parser)
{
    return (0);
}

CodecWsExtentJson::CodecWsExtentJson(loss::E_CODEC_TYPE eCodecType,
                ...
                const MsgBody& oMsgBody, loss::CBuffer* pBuff)
{
    ...
    else
    {
        ...
        const std::string& strJsonBody = oMsgBody.body();//原始数据
        {//payload数据
            ...
            stOutMsgHead.body_len = htonl((unsigned int) oMsgHead.ByteSize());
            stOutMsgHead.seq = htonl(oMsgHead.seq());
            stOutMsgHead.checksum = 0;//htons((unsigned short)stOutMsgHead.checksum);
            ...
            {
                stOutMsgHead.body_len = strJsonBody.size();
            }
        }
        ...
        LOG4_TRACE("oMsgBody.ByteSize() = %d, sizeof(stOutMsgHead) = %d,iHadWriteLen = %d(compress or encrypt maybe)",
                            oMsgBody.ByteSize(),sizeof(stOutMsgHead), iHadWriteLen);
    }
    return (CODEC_STATUS_OK);
}

E_CODEC_STATUS CodecWsExtentJson::Encode(const HttpMsg& oHttpMsg,
                loss::CBuffer* pBuff)
{
...
E_CODEC_STATUS CodecWsExtentJson::EncodeHandShake(const HttpMsg& oHttpMsg,
                loss::CBuffer* pBuff)
{
...
E_CODEC_STATUS CodecWsExtentJson::EncodeHttp(const HttpMsg& oHttpMsg,
                loss::CBuffer* pBuff)
{
    LOG4_TRACE("%s() pBuff->ReadableBytes() = %u, ReadIndex = %u, WriteIndex = %u",
...
const std::string& CodecWsExtentJson::ToString(const HttpMsg& oHttpMsg)
{
    LOG4_TRACE("%s()", __FUNCTION__);
...
{
    if (pBuff->ReadableBytes() >= 5)
    {
        //响应的是("HTTP/", 5)
        //请求的是("GET ", 4)  ("POST ", 5)
        const char* pReadAddr = pBuff->GetRawReadBuffer();//处理http请求
        if ((memcmp(pReadAddr, "GET ", 4) == 0)
                        || memcmp(pReadAddr, "POST ", 5) == 0)
        {
            LOG4_TRACE("%s() pBuff->ReadableBytes() = %u:%s", __FUNCTION__,
                                        pBuff->ReadableBytes(), pBuff->ToString().c_str());
            HttpMsg oHttpMsg;
