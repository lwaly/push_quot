...
namespace bsw
{
    ...
    DataProxyClient::DataProxyClient()
        : m_uiDProxyCmd(0), m_uiSequenece(0), m_pLabor(NULL), m_pLogger(NULL),
        m_bDataProxyEvent(true), m_iReqCount(0), m_uiReqSum(0)
    {
            ...
            do
            {
                ...
                if (oErrInfo.error_code() == ERR_OK)
                {
                    ...
                    if (oErrInfo.error_code() == ERR_OK)
                    {
                        ...
                        {
                            std::string strMsg = FormatString("data proxy callback error [%s]", oRsp.err_msg().c_str());
                            SET_TMP_ERR_INFO(ERR_DATA_PROXY_CALLBACK, strMsg.c_str());
                        }
                    }
                }
            } while (0);
    ...
    bool DataProxyClient::SendToDataProxy(uint32 uiReqCmd, const std::string& strBuf)
    {
        ...
        if (uiReqCmd % 2 == 0) //req 都是单数的
        {
            LOG4_ERROR("invalid req!, req = %u", uiReqCmd);
            return false;
        }
    ...
    uint32 DataProxyClient::AccessToLogic(uint32 uiReqCmd, const MsgBody& oMsgBody)
    {
        ...
        if (0 == uiReqCmd || 0 == oMsgBody.ByteSize() || m_uiReqSum++ >= MAX_REQ_COUNT)
        {
