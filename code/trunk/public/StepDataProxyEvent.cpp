...
StepDataProxyEvent::StepDataProxyEvent(const CContext& oInContext) 
    : StepDataProxyEventBase(oInContext), m_uiStep(0), m_bHandleError(false),
    m_pMySession(NULL), m_pOtherSession(NULL), m_pSysSession(NULL)
{
}
...
oss::E_CMD_STATUS StepDataProxyEvent::Emit(common::errorinfo& oErrInfo)
{
    ...
    if (m_oDpClient.GetReqCount() <= 0) //如果步骤结束了，就正常退出，有可能 session 已经有数据，不需要触发存储的接口。
...
// 	return ResponseToClient(oJson.ToString());
    //tcp 不像 http，要有 protobuf 结构体传递进来
    return false;
}
...
common::errorinfo* StepDataProxyEvent::GetSysCode(const common::errorinfo& oErrInfo)
{
